...
var config = { save: true, loglevel: 'error' }; // force: true, loglevel: 'silent'
...
module.exports = function(spec) {
  ...
  var uninstall = function(module, callback) {
    ...
    if (!module) {
      async.series([
        async.apply(fs.remove, path.join(spec.path, 'node_modules')),
        async.apply(fs.writeFile, pkgPath, pkgTpl)
      ], callback);
