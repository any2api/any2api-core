...
module.exports = function() {
      ...
      async.series([
        function(callback) {
          ...
          // Executable is a directory
            ...
            if (path.resolve(exec) !== path.resolve(execPath) && args.copy) {
   ...
   * Generate API spec
   */
  obj.gen = obj.generate = function(args, done) {
    args = args || {};
    ...
    args.specPath = path.resolve(args.specPath);
    ...
    if (S(args.outputPath).startsWith(args.specPath)) {
      return done(new Error('output directory cannot be sub-directory of API spec path'));
    }
    ...
    async.series([
      function(callback) {
        util.readSpec({ specPath: args.specPath }, updateSpecCallback(callback));
      },
      function(callback) {
          ...
          if (apiSpec.generated) {
            ...
            if (g.supports(apiSpec)) {
              ...
              g.generate(apiSpec, function(err) {
                ...
                var title = apiSpec.implementation.title || 'untitled';
                var description = apiSpec.implementation.description || '';
                var ports = apiSpec.implementation.ports || [ '8080' ];

                var dockerBaseImg = process.env.DOCKER_BASE_IMAGE || 'node:4';
                ...
                async.parallel([
                  ...
                  function(callback) {
                    fs.readFile(path.join(__dirname, '..', 'tpl', 'docker-compose.yml.tpl'), 'utf8', function(err, content) {
                      if (err) return callback(err);

                      fs.writeFile(path.join(implPath, 'docker-compose.yml'),
                        _.template(content, { ports: ports, serviceName: apiSpec.implementation.title }), callback);
                    });
                  },
                  function(callback) {
                  ...
                  function(callback) {
                    if (fs.existsSync(path.join(implPath, 'README.md'))) return callback();

                    fs.readFile(path.join(__dirname, '..', 'tpl', 'README.tpl'), 'utf8', function(err, content) {
                      if (err) return callback(err);

                      fs.writeFile(path.join(implPath, 'README.md'),
                        _.template(content, { ports: ports, title: title,
                                              description: description,
                                              name: S(title.toLowerCase()).camelize().s }), callback);
                    });
                  },
      ...
      function(callback) {
        ...
        async.eachSeries(_.keys(apiSpec.executables), function(name, callback) {
          ...
          util.prepareBuildtime({ apiSpec: apiSpec, preparedInvokers: preparedInvokers, executableName: name }, callback);
        }, callback);
      },
      function(callback) {
        if (args.skipPrepare) return callback();
        ...
        async.eachSeries(_.keys(apiSpec.executables), function(name, callback) {
          ...
          util.prepareExecutable({ apiSpec: apiSpec, executableName: name }, updateSpecCallback(callback));
        }, callback);
      }
    ], function(err) {
      if (err) return done(err);
      ...
      util.writeSpec({ apiSpec: apiSpec }, function(err, updApiSpec) {
        ...
        log('API implementation generated with API spec: ' + apiSpec.path);
